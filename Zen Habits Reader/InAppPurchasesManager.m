...
NSString* const UnlockedAllPostsPreferencesKey = @"UnlockedAllPostsPreferencesKey";
...
- (BOOL) allPostsAreAvailable
    {
    NSNumber* postsAvailable = [[NSUserDefaults standardUserDefaults] objectForKey:UnlockedAllPostsPreferencesKey];
...
- (void) requestProductInfoInViewController:(UIViewController*)viewController
    ...
    if ([SKPaymentQueue canMakePayments])
        {
        NSSet<NSString*>* identifiers = [[NSSet alloc] initWithObjects:InAppPurchaseIdentifier, nil];
...
- (void) presentPurchaseDialogInViewController: (UIViewController*)viewController
    ...
    if ([self allPostsAreAvailable])
        {
        [self presentAlertWithTitle:@"Already Purchased" andText:@"You have already purchased this upgrade"];
        return;
        }
        
    UIAlertController* alertController =  [UIAlertController alertControllerWithTitle:@"Locked" message:@"Only the Starter's Guide is available until you purchase the Unlock All Posts upgrade. Would you like to purchase it now?" preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction* buyAction = [UIAlertAction actionWithTitle:@"Buy" style:UIAlertActionStyleDefault handler:^(UIAlertAction* action)
         ...
         SKPayment* payment = [SKPayment paymentWithProduct:_unlockAllPostsProduct];
...
- (void)restoreCompletedTransactions
    ...
    if ([self allPostsAreAvailable])
        {
        [self presentAlertWithTitle:@"Already Unlocked" andText:@"The Unlock All Posts upgrade is already active."];
        return;
        }
    ...
    for (SKPaymentTransaction* transaction in transactions)
        ...
        switch (transaction.transactionState) {
            case SKPaymentTransactionStatePurchased:
                ...
                [self presentAlertWithTitle:@"Unlock All Posts" andText:@"All posts are now unlocked"];
                [[SKPaymentQueue defaultQueue] finishTransaction:transaction];
                self.TransactionInProgress = false;
                [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] forKey:UnlockedAllPostsPreferencesKey];
