...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"ZenHabitsReader.sqlite"];
    
    //Pre-load DB
    if (![[NSFileManager defaultManager] fileExistsAtPath:[storeURL path]])
        {
        NSURL *preloadURL = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"ZenHabitsReader" ofType:@"sqlite"]];
        NSError* err = nil;
...
- (void)saveContext
{
    ...
    //dispatch_async(dispatch_get_main_queue(), ^{
        NSError *error = nil;
        NSManagedObjectContext *managedObjectContext = isBackground ? self.backgroundManagedObjectContext : self.managedObjectContext;
        if (managedObjectContext != nil) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:NSManagedObjectContextDidSaveNotification object:managedObjectContext];
        }
}
...
- (Month *) getOrCreateUniqueMonth:(NSString *)monthString yearForMonth:(Year *)year
    {
    ...
    Month *uniqueMonth;
        ...
        if (!request)
            ...
            request.entity = [NSEntityDescription entityForName:@"Month" inManagedObjectContext: context];
            }

    request.predicate = [NSPredicate predicateWithFormat:@"(year.theYear = %@) AND (monthOfYear = %@)", year.theYear, monthString];
    
    uniqueMonth = [[context executeFetchRequest:request error:&executeFetchError] lastObject];
    
    if (executeFetchError)
        {
        NSLog(@"[%@, %@] error looking up month with month string: %@ and year string: %@ with error: %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), monthString, year.theYear, [executeFetchError localizedDescription]);
        }
    else if (!uniqueMonth)
        {
        ...
        uniqueMonth.date = [KGNUtilities dateFromFriendlyString: [NSString stringWithFormat:@"1 %@ %@", monthString, year.theYear]];
...
- (NSArray *) allPostHeadersWithSortDescriptor: (NSSortDescriptor*)sortDescriptor andPredicate: (NSPredicate*)predicate
    {
    NSError *error;
    ...
    if (sortDescriptor)
        {
        fetchRequest.sortDescriptors = [[NSArray<NSSortDescriptor *> alloc] initWithObjects:sortDescriptor, nil];
        }
...
#ifdef DEBUG
//    for (PostHeader *info in fetchedObjects)
//        {
...
- (NSArray *) allMonths
{
    NSError *error;
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"Month"
                                              inManagedObjectContext:[self managedObjectContext]];
    [fetchRequest setEntity:entity];
    NSArray *fetchedObjects = [[self managedObjectContext] executeFetchRequest:fetchRequest error:&error];
    ...
    for (Month *month in fetchedObjects) {
        if ([month.monthOfYear isEqualToString:@"January"] && [month.year.theYear isEqualToString:@"2015"])
        {
        NSLog(@"\n*****************Month*****************\n");
...
- (NSArray *) allNewPostHeaders
    {
    NSDate* checkDate = (NSDate*)[[NSUserDefaults standardUserDefaults] objectForKey:@"firstCheckDate"];
    if (!checkDate)
        {
        NSLog(@"No firstCheckDate!!");
        checkDate = [NSDate date]; // firstCheckDate is not set yet. Set it to now, so we fetch nothing.
        }
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(isRead = NO) AND (date > %@) AND (isNew = YES)", checkDate];
...
- (Year *) getYear:(NSString *)yearString
{
    ...
    if (!request)
        {
        request = [[NSFetchRequest alloc] init];
        request.entity = [NSEntityDescription entityForName:@"Year" inManagedObjectContext: context];
        }
    
    request.predicate = [NSPredicate predicateWithFormat:@"theYear = %@", yearString];
    
    uniqueYear = [[context executeFetchRequest:request error:&executeFetchError] lastObject];
    
    if (executeFetchError)
        {
        NSLog(@"[%@, %@] error looking up year with year string: %@ with error: %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), yearString, [executeFetchError localizedDescription]);
        }
...
- (PostHeader*) getMostRecentPost
    ...
    postCollection = [self allPostHeadersWithSortDescriptor:sortDescriptor andPredicate:nil];
...
- (void) createPostHeaderWithTitle: (NSString*)title andUrl: (NSString*)url andDay: (NSString*)day andMonth:(NSString*)month andYear:(NSString*)year andIsNew: (BOOL)isNew andIsRead: (BOOL)isRead andPostID: (NSString*)postID
    ...
    if (!request)
        {
        request = [[NSFetchRequest alloc] init];
        request.entity = [NSEntityDescription entityForName:@"PostHeader" inManagedObjectContext: context];
        }
    ...
    if (executeFetchError)
        {
        NSLog(@"[%@, %@] error looking up year with postID string: %@ with error: %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), postID, [executeFetchError localizedDescription]);
        }
    else if (!postHeader)
        {
        ...
        postHeader = [NSEntityDescription
                      insertNewObjectForEntityForName:@"PostHeader"
                      inManagedObjectContext:context];

        postHeader.title = [title stringByReplacingOccurrencesOfString:@"\r\n\t\t\t\t\t" withString:@""]; //TODO: Do something better than this
        postHeader.date = [KGNUtilities dateFromFriendlyString:[NSString stringWithFormat:@"%@ %@ %@", day, month, year]];
        ...
        {
            NSLog(@"Trying to create a post with an existing ID! Existing title: %@. New title: %@", postHeader.title, title);
        }
}
...
- (PostHeader*) getPostHeaderWithPredicate: (NSPredicate*)predicate
    ...
    if (executeFetchError)
        {
        NSLog(@"%s:Could not execute fetch request. Error: %@", __PRETTY_FUNCTION__, executeFetchError.description);
        }
