...
+ (void) loadPosts
{
    NSNumber *isPopulated    = [[NSUserDefaults standardUserDefaults] objectForKey:populatedKey];
    if (isPopulated == nil)     // App first run: download posts.
        {
        PostHeader* mostRecentPost;
        mostRecentPost = [[PersistenceManager sharedInstance]getMostRecentPost];
        NSInvocationOperation* firstDownloadOperation = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(downloadAllPostsAfterDate:) object:mostRecentPost.date];
        firstDownloadOperation.completionBlock = ^
            {
            NSLog(@"All posts: %lu", (unsigned long)[[[PersistenceManager sharedInstance] allPostHeaders] count]);
            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:firstCheckDate];
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] forKey:populatedKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
            };
        
        [[[KGNConcurrencyManager sharedInstance] backgroundOperationQueue] addOperation:firstDownloadOperation];
        }
    else
        {
...
+ (BOOL)downloadAllPostsAfterDate: (NSDate*)dateToCheck
{
    ...
    dateOfFirstCheck = (NSDate*)[[NSUserDefaults standardUserDefaults] objectForKey:firstCheckDate];
    
    foundNewPosts = false;
    
    NSURL *archiveUrl = [NSURL URLWithString:@"http://www.zenhabits.net/archives"];
    ...
    for (TFHppleElement *element in tableRowNodes) {
        ...
        if ([[[element.attributes objectForKey: @"class"] lowercaseString] isEqual: @"year"] && [element hasChildren])
            {
        ...
        isNew = NO;
        postDate = [KGNUtilities dateFromFriendlyString: [NSString stringWithFormat:@"%@ %@ %@", currentDay, currentMonth, currentYear]];
        ...
        if (dateOfFirstCheck != nil && [[dateOfFirstCheck earlierDate:postDate] isEqualToDate:dateOfFirstCheck])
            {
            isNew = YES;
            }
        ...
        [[PersistenceManager sharedInstance] createPostHeaderWithTitle:title andUrl:url andDay:currentDay andMonth:currentMonth andYear:currentYear andIsNew:isNew andIsRead:NO andPostID:postID];
        foundNewPosts = true;
    }
...
+ (NSString*) getPostBodyHTMLFromPostUrl: (NSString*) postURL andTitle: (NSString*) postTitle
    {
    //TODO: Use AFNetworking or     - dataTaskWithURL:completionHandler:
    NSData *postHtmlData = [NSData dataWithContentsOfURL:[NSURL URLWithString: postURL]];
    ...
    NSString* postString = [NSString stringWithFormat:@"<h2>%@</h2></br>%@", postTitle, postBody.raw];
    ...
    if (headNodes.count < 1)
        ...
        NSString* head = [NSString stringWithString:headBody.raw];
            head = [head stringByReplacingOccurrencesOfString:@"</head>" withString:@"<style>p {margin-left: 10px; margin-right: 10px; font-size: 17px;} h2 {margin-left: 5px; margin-right: 5px; margin-bottom: 0px; text-align: center;}</style></head>"];
        ...
        else
        {
            ...
            postString = [postString stringByReplacingOccurrencesOfString:subHeader withString:@""];
        }
...
+ (void) checkForNewPosts
    ...
    mostRecentPost = [[PersistenceManager sharedInstance] getMostRecentPost];
    mostRecentPostDate = mostRecentPost ? mostRecentPost.date : [NSDate dateWithTimeIntervalSince1970:0];
    ...
    if ([KGNUtilities isDateBeforeToday:mostRecentPostDate])
        {
        NSInvocationOperation* checkNewPostsOperation = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(downloadAllPostsAfterDate:) object:mostRecentPostDate];
            
            [[[KGNConcurrencyManager sharedInstance] backgroundOperationQueue] addOperation:checkNewPostsOperation];
        }
    }
