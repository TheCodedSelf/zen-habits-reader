...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    {
        PostWebViewController* destinationPostWebViewController = segue.destinationViewController;
        destinationPostWebViewController.currentPostHeader = self.selectedPostHeader;
    }
    else
    {
...
- (void)configureCell:(PostTableViewCell *)postCell atIndexPath:(NSIndexPath *)indexPath
{
    PostHeader *postHeader = [_fetchedResultsController objectAtIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.selectedPostHeader = [[PersistenceManager sharedInstance] getPostHeaderWithTitle:cell.postTitle.text];
    [self performSegueWithIdentifier:ToIndividualPostSegue sender:self];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        self.selectedPostHeader.isRead = [NSNumber numberWithBool:YES];
