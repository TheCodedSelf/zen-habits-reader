...
typedef NS_ENUM(NSInteger, AboutTableSections) {
    Purchases,
    ContactUs,
    Other
};
...
- (void)viewDidLoad
    ...
    [[NSBundle mainBundle] loadNibNamed:@"AboutHeaderView" owner:self options:nil];
    [[NSBundle mainBundle] loadNibNamed:@"AboutFooterView" owner:self options:nil];
    ...
    self.tableView.tableFooterView.bounds = CGRectMake(0, 0, width, screenHeight - height);
    CGRect frame = self.tableView.tableFooterView.frame;
    CGRect bounds = self.tableView.tableFooterView.bounds;
    self.tableView.tableFooterView.frame = CGRectMake(frame.origin.x, frame.origin.y, bounds.size.width, bounds.size.height);
...
- (void) prepareFeedbackMail
    {
        if([MFMailComposeViewController canSendMail])
            ...
            [mailCont setSubject:[NSString stringWithFormat:@"Zen Breath Feedback %@", [NSDate date]]];
            [mailCont setToRecipients:[NSArray arrayWithObject:@"galaxyplansoftware@gmail.com"]];
